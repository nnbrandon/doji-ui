...
export function CandlestickChart(
  data,
  {
    date = (d) => new Date(d.Date), // given d in data, returns the (temporal) x-value
  ...
  const weekdays = (start, stop) => {
    return d3.utcDays(start, +stop + 1).filter((d) => {
